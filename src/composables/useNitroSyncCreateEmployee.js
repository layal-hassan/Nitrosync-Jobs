...
const createEmployeeEndpoint = buildNitroSyncEndpoint('/v1/employees/create')
...
export const buildCreateEmployeeFormData = (payload = {}, options = {}) => {
  const companyId = normalizeValue(options.relatedCompany ?? payload.relatedCompany, 'b00af2a4-2d77-432b-bd93-4e7ea120d154')
  const planId = normalizeValue(options.planId ?? payload.planId, '2')
  const departmentId = normalizeValue(options.departmentId ?? payload.department)
  ...
  appendValue(formData, 'plan_id', planId)
  appendValue(formData, 'employee_id', payload.employeeId)
  ...
  appendValue(formData, 'personal_email', payload.personalEmail)
  ...
  appendOptionalValue(formData, 'iban', payload.iban)

  appendOptionalValue(formData, 'health_insurance_provider', payload.healthInsuranceProvider)
  appendOptionalValue(formData, 'policy_number', payload.healthPolicyNumber)
  appendOptionalValue(formData, 'coverage_level', payload.healthCoverageLevel)
  appendOptionalValue(formData, 'health_insurance_start_date', payload.healthStartDate)
  appendOptionalValue(formData, 'health_insurance_end_date', payload.healthEndDate)
  appendOptionalValue(formData, 'coverage_details', payload.healthCoverageDetails)
  appendOptionalValue(formData, 'retirement_plan_type', payload.retirementPlanType)
  appendOptionalValue(formData, 'retirement_coverage_level', payload.retirementCoverageLevel)
  appendOptionalValue(formData, 'contribution_type', payload.retirementContributionType)
  appendOptionalValue(formData, 'contribution_amount', payload.retirementContributionAmount)
  appendOptionalValue(formData, 'contribution_currency', payload.retirementContributionCurrency)
  appendOptionalValue(formData, 'employer_contribution_type', payload.employerContributionType)
  appendOptionalValue(formData, 'employer_contribution_amount', payload.employerContributionAmount)
  appendOptionalValue(formData, 'employer_contribution_currency', payload.employerContributionCurrency)
  appendOptionalValue(formData, 'other_benefits_coverage_level', payload.otherBenefitsCoverageLevel)
  appendOptionalValue(formData, 'life_insurance', payload.lifeInsurance)
  appendOptionalValue(formData, 'paid_time_off', payload.paidTimeOff)
  appendOptionalValue(formData, 'wellness_programs', payload.wellnessPrograms)
  appendOptionalValue(formData, 'details', payload.details)
  appendOptionalValue(formData, 'miscellaneous_start_date', payload.miscStartDate)
  appendOptionalValue(formData, 'miscellaneous_end_date', payload.miscEndDate)
  appendOptionalValue(formData, 'payment_method', payload.paymentMethod)
  appendOptionalValue(formData, 'performance_bonuses', payload.performanceBonuses)
  ...
  appendOptionalValue(formData, 'health_insurance_deduction', payload.healthInsurance)
  appendOptionalValue(formData, 'health_insurance_retirement_contributions', payload.retirementContributions)
  appendOptionalValue(formData, 'other_deductions_names', payload.deductionName)
  appendOptionalValue(formData, 'other_deductions_amount', payload.deductionAmount)
  appendOptionalValue(formData, 'other_deductions_currency', payload.deductionCurrency)

  appendOptionalValue(formData, 'attach_name', payload.employeeId || `${normalizeValue(payload.firstName)} ${normalizeValue(payload.lastName)}`.trim())
  ...
  appendFiles(formData, 'enrollment_contracts[]', payload.employmentContractFiles)
...
export const createNitroSyncEmployee = async (
  payload,
  {
  ...
  console.debug(
    '[NitroSync create employee] FormData entries',
  ...
  try {
    const response = await axios.post(createEmployeeEndpoint, formData, { timeout })
    console.debug('[NitroSync create employee] response.data', response?.data)
    const responseCode = String(response?.data?.code ?? '').trim()
    const responseMessage = String(response?.data?.message ?? '').trim()

    if (responseCode === '0') {
      throw new Error(responseMessage || 'Failed to create employee.')
    }

    return {
      code: responseCode,
      message: responseMessage || 'Employee created successfully.',
      data: response?.data?.data ?? null,
    }
  } catch (error) {
    console.debug('[NitroSync create employee] error.response.data', error?.response?.data)
    throw new Error(getNitroSyncErrorMessage(error, 'Failed to create employee.'))
  }
}

export { createEmployeeEndpoint }
